...
int DylIA::evaluate(Board board, bool isJ1, bool AIPlaying, int depth, int depthMax) {
    return BaseIA::evaluate(board, isJ1, AIPlaying, depth, depthMax);
//    int x;
//
...
bool custome_compare(const std::pair<int, pair<int, bool>> &p1, const std::pair<int, pair<int, bool>> &p2) {
...
int DylIA::minmax_alphaBeta(Board &currentBoard, bool AIPlaying, int depth, int depthMax, long long int *acc, bool isJ1,
    ...
    if (depthMax / 2 == depth) {
        ...
        for (int i = 0; i < SIZE; i++) {
            for (int colorJ = 0; colorJ < 2; colorJ++) {
                ...
                if (currentBoard.checkValidMove(i, isRed)) {
                    if (isRed) {
                        moves[i] = make_pair(evaluate(currentBoard, isJ1, AIPlaying, depth, depthMax),
                                             make_pair(i, isRed));
                    } else {
                        moves[i + SIZE] = make_pair(evaluate(currentBoard, isJ1, AIPlaying, depth, depthMax),
                                                    make_pair(i, isRed));
                    }
        ...
        std::sort(std::begin(moves), std::end(moves), custome_compare);
//        printf("\n---------------------\n");
//        for (int i = 0; i < TAB_VALUES_SIZE; i++) {
//            printf(" {%d, {%d, %d}} ", moves[i].first, moves[i].second.first, moves[i].second.second);
//        }
//        printf("\n\n\n");
        if (AIPlaying) {
            bestMove = -100;
